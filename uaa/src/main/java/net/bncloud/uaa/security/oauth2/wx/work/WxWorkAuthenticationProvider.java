...
public class WxWorkAuthenticationProvider extends AbstractAuthenticationProvider {

    public WxWorkAuthenticationProvider() {
    }
    ...
    protected final UserDetails retrieveUser(Object principal, Authentication authentication)
            throws AuthenticationException {
        try {
            UserDetails loadedUser = this.getLoginUserDetailsService().getUserByWxWorkAuthCode((String) principal);
            if (loadedUser == null) {
    ...
    protected AbstractAuthenticationToken doCreateSuccessAuthentication(Object principal, Object credentials, Collection<? extends GrantedAuthority> authorities) {
        return new WxWorkAuthentication(principal, credentials, authorities);
    }

    @Override
    public boolean supports(Class<?> authentication) {
        return WxWorkAuthentication.class.isAssignableFrom(authentication);
    }
}
